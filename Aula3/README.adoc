...
https://www.w3schools.com/python/python_classes.asp[tutorial]
...
Escreva um programa que realize a medição do tempo que demorou a executar um ciclo que calcula a raiz quadrada dos números até 50 milhões. O programa deve imprimir a data atual ao iniciar e depois uma mensagem com o tempo que demorou a calcular as raízes quadradas.
...
A utilização de _namedtuples_ pode ser vantajosa por comparação com muitoas outras abordagens. No entanto, no código desenvolvido no exercício 3 há ainda um ponto pouco claro. As funções respeitantes aos númeoros complexos estão isoladas. Claro, neste caso isto não é um grande problema visto que não há outras funções, mas imagine que tinha outras 50 funções dedicadas a outras tarefas. Quando assim é, importa organizar a informação e as funcionalidades em estruturas de dados que encapsulem um determinado assunto.
